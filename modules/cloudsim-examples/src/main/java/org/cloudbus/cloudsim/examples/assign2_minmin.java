...
public class assign2_minmin {
    public static void main(String[] args) throws Exception {
        ...
        //Always assign the task with the minimum earliest completion time to the machine that gives this minimum.
        ...
        while (!remaining.isEmpty()) {
            ...
            for (TaskInfo task : remaining) {
                ...
                for (VmInfo vm : vmInfos) {
                    double ct = readyTime[vm.id] + ((double) task.length / vm.mips);//completion time on the VM
            ...
            bestTask.cloudlet.setVmId(bestVm);// the task that is assigned first to the VM , run first in simulation
            readyTime[bestVm] = bestCT;//setting the time to that VM
